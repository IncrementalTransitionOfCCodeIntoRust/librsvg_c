...
impl AspectRatio {
    pub fn from_u32 (val: u32) -> AspectRatio {
        ...
        let defer = val.contains (AspectRatioFlags::DEFER);

        let mut aligned: bool = true;

        let align: AlignMode = {
            if val.contains (AspectRatioFlags::XMIN_YMIN)      { AlignMode::XminYmin }
            else if val.contains (AspectRatioFlags::XMID_YMIN) { AlignMode::XmidYmin }
            else if val.contains (AspectRatioFlags::XMAX_YMIN) { AlignMode::XmaxYmin }
            else if val.contains (AspectRatioFlags::XMIN_YMID) { AlignMode::XminYmid }
            else if val.contains (AspectRatioFlags::XMID_YMID) { AlignMode::XmidYmid }
            else if val.contains (AspectRatioFlags::XMAX_YMID) { AlignMode::XmaxYmid }
            else if val.contains (AspectRatioFlags::XMIN_YMAX) { AlignMode::XminYmax }
            else if val.contains (AspectRatioFlags::XMID_YMAX) { AlignMode::XmidYmax }
            else if val.contains (AspectRatioFlags::XMAX_YMAX) { AlignMode::XmaxYmax }
            else {
                aligned = false;
                AlignMode::XmidYmid
            }
        ...
        let fit: FitMode = if val.contains(AspectRatioFlags::SLICE) { FitMode::Slice } else { FitMode::Meet };
    ...
    pub fn to_u32 (&self) -> u32 {
        ...
        if self.defer { val = val | AspectRatioFlags::DEFER; }
        ...
        match self.align {
            ...
            Align::Aligned { align, fit } => {
                match align {
                    AlignMode::XminYmin => { val = val | AspectRatioFlags::XMIN_YMIN; },
                    AlignMode::XmidYmin => { val = val | AspectRatioFlags::XMID_YMIN; },
                    AlignMode::XmaxYmin => { val = val | AspectRatioFlags::XMAX_YMIN; },
                    AlignMode::XminYmid => { val = val | AspectRatioFlags::XMIN_YMID; },
                    AlignMode::XmidYmid => { val = val | AspectRatioFlags::XMID_YMID; },
                    AlignMode::XmaxYmid => { val = val | AspectRatioFlags::XMAX_YMID; },
                    AlignMode::XminYmax => { val = val | AspectRatioFlags::XMIN_YMAX; },
                    AlignMode::XmidYmax => { val = val | AspectRatioFlags::XMID_YMAX; },
                    AlignMode::XmaxYmax => { val = val | AspectRatioFlags::XMAX_YMAX; },
                }

                match fit {
                    FitMode::Meet  => { },
                    FitMode::Slice => { val = val | AspectRatioFlags::SLICE; }
                }
            }
        }
